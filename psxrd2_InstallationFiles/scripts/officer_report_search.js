// javascript used in the page of report search to send the search key to the mysql query.
...
window.onload = function () {
    ...
    function login(search, name, onSuccess, onError) {
        ...
        request.open('post', '../php/officer_report_search.php', true)
        ...
        request.onreadystatechange = function () {
            if (request.readyState === 4) {
                ...
                if (request.status === 200) {
                    ...
                    onSuccess(response.search, response.error)
    ...
    function handleSuccess(search, results) {
        sessionStorage.setItem("reports", search);
        sessionStorage.setItem("numberResults", results);
        window.location.href = "officer_report_results.html";
    }
